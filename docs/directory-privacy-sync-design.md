...
| 能力 | 是否需要修改 | 证据 | 优先级 |
| --- | --- | --- | --- |
| 目录隐私 | 需要 | 改造前，`POST /api/search/suggest` 向具有 REST read 权限的调用者返回全工作区用户的 `email`，且用户/群组候选缺少业务上下文。 | P0 隐私止损，P1 完整策略 |
| 同步状态 | 需要 | 改造前，客户端已有 Hocuspocus/Yjs/y-indexeddb 事件，但只在连接中断 5 秒后显示一个无线网警告；`Ctrl/Cmd+S` 无反馈，新版本刷新不检查本地待同步变更。 | P1 |

实施状态：两项能力均已在 `d3599df9` 完成，以下现状和风险描述保留为设计依据。
