...
| 成员目录隐私                 | 已完成           | SQL 级上下文过滤、最小 DTO、owner 策略和限流已落地 |
...
| 空间关系图                   | 已完成受限版本   | 页面级权限、懒加载、触摸交互和大空间上限已落地   |
...
| 协作同步状态                 | 已完成           | 状态、重试、刷新保护和真实断网恢复测试已落地     |
...
- [x] `BUG-FM-015` 避免移除当前页面时短暂渲染回收站横幅，并在空间侧边栏增加常驻回收站入口。
...
### 2026-07-13：成员目录隐私与协作同步状态（`d3599df9`）
