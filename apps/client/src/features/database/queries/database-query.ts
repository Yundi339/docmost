...
export function useUpdateDatabaseRecordMutation(databaseId?: string) {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (input: { recordId: string; fields: Record<string, unknown> }) =>
...
export function useReorderDatabaseRecordMutation(databaseId?: string) {
  ...
  return useMutation({
    ...
    onMutate: async (input) => {
      await queryClient.cancelQueries({ queryKey: ['database-records', databaseId] });
      ...
      queryClient.setQueryData<DatabaseRecord[]>(
        ['database-records', databaseId],
        (records = []) => reorderRecords(records, input.recordId, input.beforeRecordId),
      );
    ...
    onSettled: () => {
      void queryClient.invalidateQueries({ queryKey: ['database-records', databaseId] });
    },
  });
}
...
export function useDetachDatabaseRecordMutation(databaseId?: string) {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (input: { recordId: string; targetPageId?: string }) =>
    ...
    onMutate: async (input) => {
      ...
      queryClient.setQueryData<DatabaseRecord[]>(
        ['database-records', databaseId],
        (records = []) => records.filter((record) => record.id !== input.recordId),
      );
...
function reorderRecords(
  ...
  nextRecords.splice(insertionIndex < 0 ? nextRecords.length : insertionIndex, 0, movingRecord);
  return nextRecords;
}
...
export function useUpdateDatabaseTitleMutation(databaseId?: string) {
  ...
  return useMutation({
    ...
    onSuccess: (database) => {
      queryClient.setQueryData<DatabaseBlockInfo>(['database', databaseId], database);
    },
  });
}
