...
function nodeDOMAtCoords(
  ...
  // Custom nodes render via tiptap's React node-view
  ...
  const selectors = [
    ...
    "h5",
    "h6",
    // Tables nested in another block have a
  ...
  return document
    ...
    .find((elem: Element) => {
      // Skip elements that belong to a nested editor. Only consider elements
      // whose closest editor is this host view.
  ...
  function handleDragStart(event: DragEvent, view: EditorView) {
    ...
    if (
      ...
      if (isCustomNodeDOM(node, options)) {
        // The drag landed on a custom-node container.
