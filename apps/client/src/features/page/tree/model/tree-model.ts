...
function collectIds<T extends object>(nodes: TreeNode<T>[]): Set<string> {
    ...
    const walk = (nodes: TreeNode<T>[]): TreeNode<T>[] =>
      nodes.map((n) => {
        if (n.id === parentId) {
          ...
          // Dedup across the full loaded tree. A stale lazy-load response can
          // still include a node that has just been moved elsewhere.
          const existingIds = new Set(existing.map((c) => c.id));
          const fresh = children.filter(
            (c) => !existingIds.has(c.id) && !treeIds.has(c.id),
          );
