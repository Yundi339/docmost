...
export function SpaceTreeRow({
  ...
  const isIndeterminate =
    !isNodeSelected &&
    collectDescendantIds(node).some((id) => selectedIds.has(id));
  ...
  const handleEmojiSelect = (emoji: { native: string }) => {
    ...
    updatePageMutation.mutate({ pageId: node.id, icon: emoji.native });
  };

  const handleRemoveEmoji = () => {
    handleUpdateNodeIcon(node.id, null);
    updatePageMutation.mutate({ pageId: node.id, icon: null });
  };
  ...
  return (
    <Link
      ...
      onTouchStart={(event) => {
        ...
        const suppressContextMenu = (contextEvent: Event) => {
